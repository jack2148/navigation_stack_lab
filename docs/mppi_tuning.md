...
## 문제 4: `consider_footprint: true` 설정 시 SIGSEGV 크래시

### 증상
- `bringup_nav` 실행 후 아래 순서로 전체 nav 스택이 즉시 종료:
  1. `[WARN] Inconsistent configuration in collision checking. Please verify the robot's shape settings in both the costmap and the cost critic.`
  2. `Magick: abort due to signal 11 (SIGSEGV) "Segmentation Fault"`
  3. `[ERROR] component_container_isolated: process has died [exit code -6]`

### 원인
costmap이 `robot_radius` 기반 원형 근사로 설정되어 있는데 (`footprint` 폴리곤 미정의),
CostCritic의 `consider_footprint: true`로 설정하면 MPPI가 footprint 폴리곤 데이터에 접근하려다 세그폴트 발생.

```
costmap 설정: robot_radius: 0.28  ← 원형 근사 (footprint 없음)
CostCritic:   consider_footprint: true  ← footprint 폴리곤 접근 시도 → NULL 참조 → SIGSEGV
```

### 해결
**반드시 `consider_footprint: false` 유지.**
원형 근사 costmap 사용 시 `consider_footprint: true`는 절대 설정 불가.

```yaml
CostCritic:
  consider_footprint: false  # true로 바꾸면 SIGSEGV 크래시
```
